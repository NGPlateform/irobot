...
> 另实现了 **三维地图**（对应 Phase 2 的建图/Nav2 map_server 语义）：程序化生成障碍环境、机器人激光
> 雷达累积**占据栅格**、导航 A\* **基础避障**、地图 `state/maps/*.json` **保存/加载/清除**（2D 栅格 +
> 3D 体素渲染）。执行边界与安全不变量不变。
> 进一步把 3D 探索升级为**游戏化低多边形自然场景**且**环境即物理世界**：程序化地形（`terrainSeed` +
> 手写 value-noise/fBm 高度图，机器人贴地形起伏）、低多边形树/岩（=避障障碍）、河流折线（不可通行、
> A\* 绕行、半程溪流保连通）与随机游走小动物，全部离线程序化。并实现**基于前沿（frontier）的自主
> 探索**（对应 Phase 2 的自主建图/Nav2 探索语义）：新增绝对坐标能力 `robot.navigation.navigate_to_point`，
> `nextFrontier` 在可达域内择最近前沿，`autoExplore` 驱动舰队**完整走安全管线**自主建图直至覆盖收敛。
