...
- **🛰 3D 探索模式** — 多机器人舰队的**游戏化低多边形 3D 自然场景**：程序化起伏地形（草/坡土/近水沙
  按高度着色、`flatShading` 刻面）、低多边形树木与岩石、蜿蜒河流（半透明水面 + 正弦波纹）、随机游走
  的小动物（鹿/兔/鸟）、天空渐变 + 雾。机器人**贴地形起伏**行驶，叠加激光雷达点云、机械臂、多机高亮、
  绕障轨迹与占据栅格。视觉**完全离线程序化**（手写可种子 value-noise/fBm，无外部模型/贴图/网络）。
  详见下方「游戏化 3D 环境 + 自主探索」。
...
**服务器生成的环境就是"游戏世界"**——不是纯装饰：`生成环境` 除障碍外还生成 `terrainSeed`（地形种子）
与一条**河流折线**（`WorldMapData.river` + `riverWidth`），每个障碍带 `kind`（`tree` / `rock`）。这些
经 `map` SSE 下发，客户端 `scene3d` 据此渲染地形/树/河/动物；服务器侧同一份数据即物理约束：

- **树/岩 = 物理障碍** — 参与 A\* 膨胀避障与激光雷达建图（3D 里就是那几棵树/石头）。
- **河流不可通行** — `pointBlocked` 对到河流折线距离 `< riverWidth/2 + margin` 的点判为阻挡；A\*
  绕行、建图标为占据。为**保证连通性**，河流做成"半程溪流"（从一边进、止于地图内部，不完全横穿）。
- **机器人贴地形** — 3D 中机器人、轨迹点、激光雷达射线的高度都取 `terrainHeight(x,y)`，随丘陵起伏。

**🧭 自动探索**（工具条按钮 / `POST /explore {on}`）：基于**前沿（frontier）**的自主建图——

- 服务器 `nextFrontier(pose)`：BFS 求当前位姿可达域，在其中找**空闲且相邻未知**的最近前沿格。
- `autoExplore` 每 ~800ms 遍历舰队：对空闲机器人下发 `robot.navigation.navigate_to_point{x,y}` 目标，
  **完整走安全管线**（Orchestrator 校验 → 单写者 `base_motion` → Edge 准入 → deadline → 审计），
  机器人自主驶向前沿、A\* 绕开树/河、激光雷达持续建图，覆盖率上升。
- 所有机器人连续多轮无新前沿（可达域探完）→ 自动停止并播报「自动探索完成，地图已建好」。
  `estop` 时不下发新目标（安全）。前端按钮经 `explore` SSE 双向同步开关态。

> 新增能力 `robot.navigation.navigate_to_point`（S2_GUARDED，绝对坐标导航），复用既有
> `SimRobot.execute` 的 A\* 避障；自主探索因此天然享有与人工指令**相同的安全边界**。
